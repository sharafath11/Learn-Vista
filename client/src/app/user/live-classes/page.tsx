      ...
      {courses.length > 0 && (
        <div className="mb-6">
          <div className="flex gap-2 p-1 bg-gray-100 rounded-lg w-fit">
            <WithTooltip content="Show active sessions">
              <button
            ...
            <WithTooltip content="Show expired sessions">
              <button
            ...
            <div className="divide-y divide-gray-100">
              {filteredCourses.map((session, index) => (
                <SessionTableRow
                  ...
                  onJoinCall={() => (
                    <WithTooltip content="Join this live session">
                      <button onClick={() => handleJoinCall(session.id)}>Join</button>
                    </WithTooltip>
                  )}
                  onViewContent={() => (
                    <WithTooltip content="View session content">
                      <button onClick={() => handleViewContent(session.id)}>View</button>
                    </WithTooltip>
                  )}
                />
              ))}
