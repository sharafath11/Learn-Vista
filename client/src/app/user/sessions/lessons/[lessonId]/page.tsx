...
export default function LessonPage() {
  ...
  const updateLessonProgressInDB = useCallback(
    async (updateData: Partial<IUserLessonProgress>): Promise<boolean> => {
  ...
  const handleVideoComplete = useCallback(async () => {
    ...
    const finalVideoTotalDuration =
      videoTotalDuration > 0
        ? videoTotalDuration
        : lesson?.duration && !isNaN(parseFloat(lesson.duration.toString()))
  ...
  const handlePracticalComplete = useCallback(
    async (answers: { question: string; answer: string }[]) => {
  ...
  const submitLessonReport = useCallback(async () => {
    ...
    if (combinedAnswers.length === 0) return
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (videoProgressDebounceTimeoutRef.current) clearTimeout(videoProgressDebounceTimeoutRef.current)
    }
  }, [])
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-6">
      <div className="container mx-auto px-4 max-w-6xl">
        <WithTooltip content="Go back to the course sessions">
          <LessonHeader
            lesson={lesson}
            report={report}
        ...
        <WithTooltip content="Watch the lesson video and complete all activities">
          <LessonContent
            videoUrl={videoUrl}
        ...
        <WithTooltip content="Join the discussion and leave a comment">
          <LessonDiscussion comments={comments} videoCompleted={videoCompleted} onAddComment={handleAddComment} />
        </WithTooltip>
      </div>

      <ReportModal report={report?.report || ""} isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} />
