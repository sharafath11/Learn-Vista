...
export default function ConcernsPage() {
  ...
  }, [statusFilter, selectedCourseId, currentPage, debouncedSearchTerm, sortOrder]);

  // Call fetchConcerns when dependencies change
  useEffect(() => {
    fetchConcerns();
  }, [fetchConcerns]);
  ...
  return (
    <div className="min-h-screen px-6 py-8 bg-gray-50 dark:bg-gray-900">
                        ...
                        <div key={attachment.filename || idx} className="flex-shrink-0">
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              setMediaType(attachment.type === "audio" ? "audio" : "image");
                              setOpen(true);
                            }}
                          >
                            {attachment.type === "image" ? <ImageIcon className="w-3 h-3" /> : <FileIcon className="w-3 h-3" />}
                            {attachment.type === "image" ? "Image" : attachment.type === "audio" ? "Audio" : "File"}
                          </Button>
                        </div>
                      ))}
                    </div>
      ...
      {/* Modals */}
      <ConcernModal concern={selectedConcern} onClose={() => setSelectedConcernId(null)} onStatusChange={fetchConcerns} />
